...
\title{An Uncertainty-Aware Multimodal Evidence Fusion
Framework for Vessel-Associated Subsea Cable Disturbance
Assessment under Adversarial Uncertainty}

\author{
\IEEEauthorblockN{Muthuvel Mukesh K, Vishnu K H, Muthukrishnan C, Sairam M. S.}
...
\begin{abstract}
Subsea fiber-optic and power cables constitute critical international
infrastructure whose physical disruption impacts global communications, energy
distribution, financial transactions, and maritime logistics. Detecting a
seafloor vibration anomaly, however, does not inherently establish that an
active maritime vessel produced it, nor does it establish physical cable
damage. This paper presents an uncertainty-aware multimodal evidence fusion
framework designed to evaluate vessel-associated subsea cable disturbances under
incomplete, conflicting, and adversarial evidence. The framework explicitly
decouples physical disturbance detection, spatio-temporal association, vessel
kinematics, sensor health monitoring, counter-evidence evaluation, and
uncertainty quantification into distinct evidence streams. These streams are
integrated via a graded, fail-closed decision hierarchy producing
\dstate{T0}--\dstate{T3} operational states alongside a dedicated \dstate{TX}
state indicating degraded sensor health or severe evidence conflict. We report
an extensive empirical evaluation executed across three real submarine
distributed acoustic sensing (DAS) records: (i) an AIS-linked container-ship
transit (Marlinks, 60 windows, 250 channels, closest approach 26.2~m)
providing 1D proximity validation; (ii) an independent deep-sea background
recording (EMSO Western Ionian, 105 windows, 2,963 channels, 1,050~s) providing
deep-sea acoustic noise floor validation; and (iii) an independent multi-week
Arctic recording (Dryad Oliktok, 215 hourly windows, 183 physical channels, 32
frequency bins over 27.625 days) providing long-duration environmental
robustness and escalation-avoidance evaluation against co-located oceanographic
moorings. On Marlinks, the DAS physical channel separates 1~km corridor
incursions at AUC $=0.959$ (95\% CI $[0.887, 1.000]$), with DAS power tracking
AIS range at Spearman $\rho = -0.914$ ($p = 2.1\times10^{-24}$) and range
regression generalising at $R^2_{\log} = 0.849$. On EMSO, the framework
yielded zero false escalations across 85/85 background windows. On Oliktok, the
detector exhibited stable acoustic noise floors (mean integrated strain RMS
$550.85~\mathrm{nm/m/s}$, drift $+0.0188/\mathrm{hr}$, $p=0.8037$), showing
statistically significant empirical associations with independent significant
wave height ($\rho = +0.3141$, $p = 2.63\times10^{-6}$) and seafloor pressure
variance ($\rho = +0.4951$, $p = 1.07\times10^{-14}$), while yielding 93
\dstate{T0} (43.26\%), 122 \dstate{T1} (56.74\%), and zero \dstate{T2}/\dstate{T3}
escalations (0.0\% environmental escalation rate). Adversarial evasion rate is
0.0 across four attack families and five severities applied to the vessel-side
AIS channel. We also report our negative ablation result: at reference
thresholds the full framework achieves F1 $=0.462$ on the held-out transit and
F1 $=0.235$ in 2,200 controlled simulation trials, below simple weighted fusion
(F1 $=0.824$ and $0.333$), reflecting an intentional architectural trade-off
prioritizing fail-closed safety over unconstrained escalation recall.
Verified end-to-end processing latency averages 8.289~ms (decision engine
0.009~ms). Physical ESP32--MPU6050 hardware validation was not executed and is
scoped as future work.
\end{abstract}

\begin{IEEEkeywords}
subsea cable protection, distributed acoustic sensing, AIS, multimodal
sensor fusion, uncertainty estimation, critical infrastructure,
...
\begin{table}[!t]
\caption{Validation Component Status}
\label{tab:status}
\centering
\footnotesize
\begin{tabular}{@{}p{0.38\columnwidth}p{0.30\columnwidth}l@{}}
\toprule
...
thresholds, the full framework is outperformed on pooled F1 by simple weighted
fusion on both simulated and real data, reflecting deliberate conservatism.
\end{enumerate}
...
Distributed acoustic sensing interrogates standard optical fiber cables using
...
The Automatic Identification System provides kinematic vessel state broadcasts
used globally for maritime collision avoidance and traffic monitoring
...
disable transponders, spoof MMSI identifiers, or introduce deliberate GPS
...
between acoustic energy onset and vessel CPA time. Rather than adopting an
...
\end{equation}
ensuring that degraded or missing sensors cannot artificially inflate confidence.
...
T =
\begin{cases}
\dstate{TX}, & R \leq \theta_R \ \text{or}\ \text{(\ref{eq:consistency})}, \\
...
\centering
\footnotesize
\begin{tabular}{@{}p{0.26\columnwidth}p{0.21\columnwidth}p{0.21\columnwidth}p{0.22\columnwidth}@{}}
\toprule
...
Primary SHA-256 & \texttt{46f5d2838342} & \texttt{8af2afac41b9} & \texttt{5fa69e4cf90a} \\
\bottomrule
\end{tabular}
\end{table}
...
Acoustic strain power tracks inverse vessel distance at Spearman $\rho = -0.914$
($p = 2.1\times10^{-24}$, $n=60$). Evaluated against corridor boundaries on the
held-out leg, the coherent statistic achieves the detection AUCs in
Table~\ref{tab:auc}, reaching AUC $= 0.959$ (95\% CI $[0.887, 1.000]$) at 1~km.
...
On Dataset B, a static baseline drifts over 17 minutes, producing a 97.7\% false
alarm rate. A rolling 200-second baseline stabilizes the statistic at mean 4.15
(p95 6.01, max 6.99). Sweeping $\tau_P$ identifies $\tau_P = 7$ as the minimal
integer yielding zero background false alarms (Fig.~\ref{fig:far}). Operating
the full decision engine over Dataset B produced \dstate{TX} in 85/85 windows
and \textbf{zero} false escalations to \dstate{T2}/\dstate{T3}, verifying
fail-closed behavior when vessel corroboration is absent.

\begin{figure}[!t]
\centering
\includegraphics[width=0.42\textwidth]{figs/fig_far.png}
\caption{Background false-alarm rate versus threshold on Dataset B (85 windows,
rolling baseline). The reference threshold $\tau_P=3$ produces 100\% false
alarms, whereas $\tau_P=7$ achieves zero false alarms.}
\label{fig:far}
\end{figure}
...
\centering
\footnotesize
\begin{tabular}{@{}p{0.34\columnwidth}p{0.24\columnwidth}p{0.34\columnwidth}@{}}
\toprule
...
$r$ vs Pressure Variance & +0.4682 ($p=7.74\times10^{-13}$) & Linear bottom pressure correlation \\
...
($p = 1.07\times10^{-14}$, Pearson $r = +0.4682, p = 7.74\times10^{-13}$) against
dynamic seafloor pressure variance. These correlations demonstrate empirical
coupling with surface hydrodynamics. They do not demonstrate causal cable
damage, as environmental records cannot establish causal physical damage
without annotated structural incident ground truth.
...
\begin{table}[!t]
\caption{Method Comparison on Held-Out Transit Leg ($n=33$, $R_c=1000$~m)}
\label{tab:methods}
\centering
\footnotesize
\begin{tabular}{@{}lcccc@{}}
...
physical\_only & 0.727 & 0.526 & \textbf{1.000} & 0.690 \\
...
decisions on the clean transit leg. Removing behavioral scoring increases F1
from 0.462 to 0.571, whereas removing spatio-temporal association collapses F1
to 0.000, confirming that association gating is the primary load-bearing module.
...
The adversarial evaluation perturbed the real AIS channel across 10 genuine
incursion windows while leaving the real fiber recording untouched. Four attack
families were evaluated at five severities: AIS range inflation, transponder
suppression, timestamp offset, and combined multi-vector evasion
(Table~\ref{tab:adv}).
...
The adversarial evasion rate (AER) was 0.0 across all 20 cells: within the
implemented vessel-side attack model, no trial collapsed to \dstate{T0}.
Mean uncertainty rose from 0.225 to 0.436 under full attack, and the consistency
check fired monotonically with severity for range manipulation
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.46\textwidth]{figs/fig_adversarial.png}
...
demonstrating graceful degradation. Conversely, extreme synthetic Gaussian noise
artificially inflated $R$, highlighting that high unmodeled noise can displace
windows from \dstate{TX}.

\begin{figure}[!t]
\centering
\includegraphics[width=0.42\textwidth]{figs/fig_robustness.png}
\caption{Channel dropout robustness on real DAS. F1 remains constant while
\dstate{TX} declarations rise, reflecting fail-closed degradation.}
...
it exhibits overconfidence in intermediate bins (0.2--0.6). Fused score $R$
functions as an operational reliability score and must not be interpreted as a
calibrated posterior probability.

\begin{figure}[!t]
\centering
\includegraphics[width=0.40\textwidth]{figs/fig_calibration.png}
\caption{Reliability calibration curve of fused score $R$ on the held-out
transit leg. Marker area denotes bin sample occupancy.}
...
\toprule
Processing Stage & Mean (ms) & Median & P95 & Max & Throughput \\
\midrule
...
\begin{tabular}{@{}lccccc@{}}
\toprule
Method & Accuracy & Precision & Recall & F1 & TX Rate \\
\midrule
physical\_only & 0.3182 & 0.0000 & 0.0000 & 0.0000 & 0.0000 \\
...
Third, the lower pooled F1 score (0.462 real, 0.235 simulation) reflects an
explicit engineering choice: prioritizing uncertainty awareness, health
gating, and fail-closed security over optimistic classification.
...
\section{Future Work: Hardware Validation Scope}
\label{sec:hardware}
...
six-axis inertial measurement units sampled at 10~Hz, deployed along a scaled
...
manipulation. By formally bounding the scope of adversarial security,
acknowledging lower pooled F1 relative to unconstrained fusion, and verifying
all claims against executable artifacts, this work establishes a rigorous,
reproducible foundation for subsea cable surveillance.

\begin{thebibliography}{00}
\bibitem{b1} ``Subsea Cable Health Monitoring System,'' IEEE Xplore, document 8168582.
\bibitem{b2} ``Subsea Cable Tracking Using Data Fusion of Signals from Array of Sensing Coils,'' IEEE Xplore, document 9530924.
\bibitem{b3} ``Autonomous Underwater Vehicles for Submarine Cable Inspection: Experimental Results,'' IEEE Xplore, document 969841.
\bibitem{b4} ``Sensing Technologies and Artificial Intelligence for Subsea Power Cable Asset Management,'' IEEE Xplore, document 9486586.
...
\bibitem{b6} ``Malicious AIS Spoofing and Abnormal Stealth Deviations: A Comprehensive Statistical Framework for Maritime Anomaly Detection,'' IEEE Xplore, document 9473225.
