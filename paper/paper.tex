...
AIS range at Spearman $\rho = -0.948$ ($p = 1.4\times10^{-30}$) and range
...
escalations (0.0\% environmental escalation rate). Adversarial evasion rate
(AER, defined as the fraction of genuine-incursion trials collapsing to
\dstate{T0} under adversarial perturbation) is
...
F1 $=0.235$ in 2,200 controlled simulation trials (evaluated separately;
these scores are not pooled), below simple weighted fusion
(F1 $=0.824$ real, $0.333$ simulation), reflecting an intentional architectural
trade-off prioritizing fail-closed safety over unconstrained escalation recall.
...
reflecting deliberate fail-closed conservatism.
\end{enumerate}
...
spatio-temporal association collapses F1 to 0.000, confirming that association
gating is the primary load-bearing module.
...
vessel-side attack model, no trial collapsed to \dstate{T0}. Mean uncertainty
...
evaluated independently) reflect an explicit engineering choice: prioritizing
uncertainty awareness, health gating, and fail-closed security over optimistic
classification.
...
acknowledging lower escalation F1 relative to unconstrained fusion (evaluated
independently on real and simulated data), and verifying all claims against
executable artifacts, this work establishes a rigorous,
